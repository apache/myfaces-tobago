...
<td colspan="${columns}"> </td>
  ...
  connectedCallback(): void {
    ...
    this.listeners.add(this, ClientBehaviors.TOBAGO_SHEET_ROW_SELECTION_CHANGE, this.syncSelected.bind(this));
...
Type: ${data.type}`);
  ...
  set selected(value: Set<number>) {
    ...
    if (fireSelectionChangeEvent) {
      this.dispatchEvent(new CustomEvent(ClientBehaviors.TOBAGO_SHEET_ROW_SELECTION_CHANGE, {
        detail: {
          selection: value,
