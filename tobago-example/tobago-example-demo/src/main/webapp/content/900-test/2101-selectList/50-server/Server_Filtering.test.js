...
it("SelectOneList: margin-right of span before filter input", function (done) {
  ...
  const tobagoFilter = querySelectorFn("tobago-filter[for='page:mainForm:selectOneList::field']");

  const test = new JasmineTestTool(done);
  test.setup(() => span().textContent.length === 0
          && tobagoSelectItems().length === 0
          && Number(tobagoFilter().getAttribute("min-chars")) === 0,
      null, "click", reset);
...
it("SelectManyList: avoid 'no valid selection' error", function (done) {
  ...
  const tobagoFilter = querySelectorFn("tobago-filter[for='page:mainForm:selectManyList::field']");
