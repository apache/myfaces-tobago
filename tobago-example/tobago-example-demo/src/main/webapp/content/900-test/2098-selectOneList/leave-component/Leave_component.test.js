...
it("Open dropdown, filter, leave component", function (done) {
  const selectOneList = elementByIdFn("page:mainForm:selectOneList");
  ...
  const hiddenSelectItems = querySelectorAllFn(".tobago-dropdown-menu[name='page:mainForm:selectOneList'] .tobago-select-item.d-none");
  ...
  test.do(() => expect(selectOneList().classList).not.toContain("tobago-focus"));
  ...
  test.do(() => expect(selectOneList().classList).toContain("tobago-focus"));
  test.do(() => expect(filterInput().value).toBe(""));
