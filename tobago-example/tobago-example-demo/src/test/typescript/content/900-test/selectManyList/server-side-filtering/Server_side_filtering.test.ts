...
test.describe("900-test/selectManyList/server-side-filtering/Server_side_filtering.xhtml", () => {
  ...
  test("SelectManyList: ajax listener", async ({page}) => {
    ...
    await expect(changeCounter).toHaveText("0");
    await expect(clickCounter).toHaveText(/^[23]$/); //expect "3" if Ajax is fast enough, otherwise "2" because Ajax queue blocks a second identical click event request
    await expect(dblclickCounter).toHaveText("1");
    await expect(focusCounter).toHaveText("1");
    await expect(blurCounter).toHaveText("0");
    await reset.focus();
    await expect(changeCounter).toHaveText("0");
    await expect(clickCounter).toHaveText(/^[23]$/);
    await expect(dblclickCounter).toHaveText("1");
    await expect(focusCounter).toHaveText("1");
    ...
    await expect(dblclickCounter).toHaveText("0");
    await expect(focusCounter).toHaveText("0");
    await expect(blurCounter).toHaveText("0");
    ...
    await selectField.dblclick();
    await expect(changeCounter).toHaveText("1");
    await expect(clickCounter).toHaveText(/^[34]$/); //expect "4" if Ajax is fast enough, otherwise "3" because Ajax queue blocks a second identical click event request
    await expect(dblclickCounter).toHaveText("1");
    await expect(focusCounter).toHaveText("1");
    await expect(blurCounter).toHaveText("0");
    await reset.focus();
    await expect(changeCounter).toHaveText("1");
    await expect(clickCounter).toHaveText(/^[34]$/);
    await expect(dblclickCounter).toHaveText("1");
    await expect(focusCounter).toHaveText("1");
    await expect(blurCounter).toHaveText("1");
  });
